...
interface ArtifactAdvancedDialogProps {
  id: string
  open: boolean
  onOpenChange: (open: boolean) => void
}

export default function ArtifactAdvancedDialog({
  id,
  open,
  onOpenChange,
}: ArtifactAdvancedDialogProps) {
  const {
    data: artifact,
  ...
  const hasError = artifactError && !artifactFetching
        ...
        <div className="flex min-h-0 flex-1 flex-col">
          {!artifact || artifactLoading ? (
            ...
            >
              ...
              >
                <ScrollArea className="min-h-0 w-full min-w-0">
                  <div className="flex min-w-0 flex-col gap-2 p-2">
                    <Artifacts.Attributes artifact={artifact} />

                    <CollapsibleCard title="Metadata" contentClass="p-0">
                      <Metadata resource={artifact} className="p-2" />
                    </CollapsibleCard>
                  </div>
                </ScrollArea>
              </TabsContent>
              ...
              >
                <InspectResource resource={artifact} />
              </TabsContent>
            </Tabs>
          )}
        </div>
