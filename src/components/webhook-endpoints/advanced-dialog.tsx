...
import { useGetWebhookEndpoint } from "@/queries/webhook-endpoints"
...
export default function AdvancedDialog({
  ...
  const {
    ...
    isFetching: webhookEndpointFetching,
    isError: webhookEndpointError,
  } = useGetWebhookEndpoint(id)

  const [tab, setTab] = useState<"attributes" | "inspect">("attributes")

  const hasError = webhookEndpointError && !webhookEndpointFetching
    ...
    <Dialog open={open && !hasError} onOpenChange={onOpenChange}>
      <DialogContent className="flex h-[calc(100dvh-2rem)] w-full flex-col overflow-hidden p-0 md:min-w-4xl">
        ...
        <div className="flex min-h-0 flex-1 flex-col">
          {!webhookEndpoint || webhookEndpointLoading ? (
            ...
            >
              ...
              >
                <ScrollArea className="min-h-0 w-full min-w-0">
                  <div className="flex min-w-0 flex-col gap-2 p-2">
                    <WebhookEndpoints.AllAttributes
              ...
              >
                <InspectResource resource={webhookEndpoint} />
              </TabsContent>
            </Tabs>
          )}
        </div>
