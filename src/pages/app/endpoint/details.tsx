...
import { useGetEndpoint, useRemoveEndpoint } from "@/queries/endpoints"
...
import { EndpointAttributeDescriptions } from "@/types/endpoints"
...
export default function EndpointDetails() {
  ...
  const { data: endpoint, isLoading, isFetching, isError } = useGetEndpoint(id)
  const deleteEndpoint = useRemoveEndpoint(id)

  const productId = endpoint?.relationships.product?.data?.id || ""
  const {
    data: product,
  ...
  const environment = endpoint?.relationships.environment?.data
  ...
  return (
    <section className="flex h-screen w-full">
                    ...
                    <div className="space-y-4 md:pr-3">
                      <Attribute.Field
                        ...
                        value={
                          <Attribute.Value
                            type="raw"
                            value={endpoint.attributes.url}
                            tooltip={EndpointAttributeDescriptions.url}
                            className="break-all"
                          />
                        }
                      />
                        ...
                        value={
                          <Attribute.Value
                            ...
                            value={signatureAlgorithmLabel(
                              endpoint.attributes.signatureAlgorithm,
                            )}
                            tooltip={
                              EndpointAttributeDescriptions.signatureAlgorithm
                            }
                          />
                        }
                      />
                    ...
                    <div className="mt-4 space-y-4 md:mt-0 md:pl-3">
                      <Attribute.Field
                        ...
                        value={
                          <Attribute.Value
                            type="raw"
                            value={endpoint.attributes.apiVersion}
                            tooltip={EndpointAttributeDescriptions.apiVersion}
                          />
                        }
                      />
                    </div>
                  ...
                  >
                    <CollapsibleMenu title="Properties" className="space-y-2">
                      <Attribute.Field
                        label="Created at"
                        value={formatDate(
                          new Date(String(endpoint.attributes.created)),
                          "PP",
                        )}
                      />
                      <Attribute.Field
                        label="Updated at"
                        value={formatDate(
                          new Date(String(endpoint.attributes.updated)),
                          "PP",
                        )}
                      />
                    </CollapsibleMenu>
      ...
      <ConfirmationModal
        ...
        open={open.delete}
        disabled={isLoading || deleteEndpoint.isPending}
